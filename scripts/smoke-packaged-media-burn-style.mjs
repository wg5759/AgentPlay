...
const sourceVideo = path.resolve(valueArg('--source') || 'C:/Users/Administrator/Videos/拼接验收/样式验收.mp4')
...
if (!fs.existsSync(sourceVideo) || !fs.existsSync(subtitleFile)) throw new Error('缺少样式验收夹具（视频/字幕）')

const profileDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentplay-packaged-style-'))
...
const evidenceDir = path.join(root, 'artifacts', 'acceptance', 'media-burn-style-packaged')
const directInstruction = `把字幕 ${subtitleFile} 烧进视频，字号大一点，放顶部，黄色`
...
function cleanup() {
  ...
  if (!resolved.startsWith(tempRoot) || !path.basename(resolved).startsWith('agentplay-packaged-style-')) throw new Error(`拒绝清理非验收目录：${resolved}`)
...
try {
  ...
  const pageResult = await session.evaluate(`(async () => {
    ...
    if (!directPlan.matched || directPlan.decision?.kind !== 'media.burn-subtitles') throw new Error('安装态样式烧录计划不合格：' + JSON.stringify(directPlan).slice(0, 300))
    const style = directPlan.decision.subtitle?.style
    if (style?.fontSize !== 'large' || style?.alignment !== 'top' || style?.color !== '黄色') throw new Error('安装态样式提取不合格：' + JSON.stringify(style))
    ...
    if (task.state !== 'completed') throw new Error(task.error || task.status || '样式烧录任务未完成')
    ...
    if (task.result?.projectCapsule?.versionCount !== 2 || task.result?.projectCapsule?.canUndo !== true) throw new Error('样式烧录没有生成可撤销项目胶囊')
    const bodyText = document.body.innerText
    return {
      version: window.aiPlayer.version,
      task,
      preview,
      uiReceiptVisible: bodyText.includes('大号字') && bodyText.includes('顶部') && bodyText.includes('黄色') && bodyText.includes('原文件与字幕文件均未改动')
    }
  })()`, true)
  ...
  if (!outputPath || !fs.existsSync(outputPath)) throw new Error('安装态样式烧录没有真实成果文件')
  const videoAfter = quickFingerprint(sourceVideo)
  const srtAfter = quickFingerprint(subtitleFile)
  if (videoBefore.bytes !== videoAfter.bytes || videoBefore.sha256 !== videoAfter.sha256) throw new Error('安装态样式烧录改动了源视频')
  if (srtBefore.bytes !== srtAfter.bytes || srtBefore.sha256 !== srtAfter.sha256) throw new Error('安装态样式烧录改动了字幕文件')
  if (pageResult.task.quality?.passed !== true) throw new Error(`安装态样式烧录质量门未通过：${JSON.stringify(pageResult.task.quality?.reasons || []).slice(0, 400)}`)
  if (!pageResult.uiReceiptVisible) throw new Error('对话框没有显示样式回执')
  ...
  if (!(outputDuration > 0) || Math.abs(outputDuration - 4) > 0.25) throw new Error(`安装态样式烧录成品时长不合格：${outputDuration}`)
  // 像素级核验：顶部带应有黄色字幕像素（YUV 有限范围压暗，用通道差谓词）
  const rgbPath = path.join(evidenceDir, 'styled-frame-2s.rgb')
  await runExecutable(ffmpegPath, ['-hide_banner', '-nostdin', '-ss', '2', '-i', outputPath, '-frames:v', '1', '-vf', 'scale=64:36', '-f', 'rawvideo', '-pix_fmt', 'rgb24', '-y', rgbPath])
  ...
  for (let y = 0; y < 36; y += 1) {
    for (let x = 0; x < W; x += 1) {
      const i = (y * W + x) * 3
      const isYellow = rgb[i] > 90 && rgb[i] - rgb[i + 2] > 50 && rgb[i + 1] - rgb[i + 2] > 50
      if (isYellow && y < 6) topYellow += 1
      if (isYellow && y >= 30) bottomYellow += 1
    }
  }
  if (!(topYellow > 10)) throw new Error(`顶部带没有黄色字幕像素：${topYellow}`)
  if (!(bottomYellow <= 2)) throw new Error(`底部带不应有黄色字幕像素：${bottomYellow}`)
  const frameProofPath = path.join(evidenceDir, 'styled-frame-2s.png')
  await runExecutable(ffmpegPath, ['-hide_banner', '-nostdin', '-ss', '2', '-i', outputPath, '-frames:v', '1', '-y', frameProofPath])
  fs.rmSync(rgbPath, { force: true })
  const persistedOutputPath = path.join(evidenceDir, 'packaged-burn-style-yellow-top.mp4')
  fs.copyFileSync(outputPath, persistedOutputPath)
  const receipt = { passed: true, checkedAt: new Date().toISOString(), executable, sourceVideo, subtitleFile, videoBefore, videoAfter, srtBefore, srtAfter, outputBytes: fs.statSync(outputPath).size, outputDuration, frameEvidence: { topYellow, bottomYellow }, frameProofPath, persistedOutputPath, screenshotPath, pageResult }
  const receiptPath = path.join(evidenceDir, 'receipt.json')
  fs.writeFileSync(receiptPath, `${JSON.stringify(receipt, null, 2)}\n`, 'utf8')
  process.stdout.write(`${JSON.stringify({ passed: true, receiptPath, screenshotPath, outputDuration, topYellow, bottomYellow, qualityScore: pageResult.task.quality?.score, persistedOutputPath, frameProofPath }, null, 2)}\n`)
} finally {
