...
test('subtitle chain prefers offline engine and exposes pack download IPC', () => {
  ...
  assert.match(main, /translateEntries\(entries, engine\.complete, \{[\s\S]{0,220}targetLang,[\s\S]{0,220}signal,[\s\S]{0,260}initialTranslations:[\s\S]{0,260}onProgress:/)
  assert.match(main, /onCheckpoint:[\s\S]{0,260}stage: 'translation-progress'/)
